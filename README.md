...
### 2.6 PPO算法
![image](https://github.com/user-attachments/assets/6c9dc349-b311-4704-a55f-321fdeb5d29c)
![image](https://github.com/user-attachments/assets/4500dc68-0d67-4a31-ab86-26e5b2ad05ac)
...
#### 2.6. 近端策略优化中的KL散度指的是什么？
本质来说，KL散度是一个函数，其度量的是两个动作间的行为距离，而不是参数距离。这里的行为距离可以理解为在相同状态下输出动作的差距（概率分布上的差距），概率分布即KL散度

#### PPO算法运用了clip函数限制取值范围，为什么还要加上min呢？

#### AC架构与PPO之间的区别？

#### PPO的上一个策略收集到的经验可以用多少次？

#### 2.6 DPO和PPO有什么区别？

#### 2.6 DPO训练可能会出现什么问题？

#### 在GRPO算法中，稳定训练的Clip方法和KL penalty方法分别是什么？GRPO同时使用Clip和KL penalty，与只用其中一种方法或是不用任何方法相比，有何优势和劣势？
...
#### 在KL惩罚方法中，固定惩罚强度和动态调整惩罚强度各有何优缺点？
